...
export default function Post({ post }) {
    const { schedules, address, location, category, createdAt, remuneration, id, description, images, idUser } = post;
    ...
    const seeProfile = () => {
        if (userInfo.idUser) {
            ...
            navigation.navigate(screen.account.seeProfile, { idUser: userInfo.idUser });
        } else {
        ...
        <Card>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
                <View style={{ marginRight: 10 }}>
                    {userInfo && (
                        <Avatar photoURL={userInfo.photoURL} onPress={seeProfile} width={50} height={50} />
                    )}
                </View>
                <View>
            ...
            <Image source={{ uri: post.images[0] }} style={{ width: "100%", height: 200 }} onPress={() => setShowModal(true)} />
            <View style={{ padding: 10 }}>
                ...
                <Text>Ubicación: {address} (a {distanceInKm})</Text>
            </View>
            <Button
                title="Ver más"
                type="outline"
            ...
            <Modal show={showModal} close={onCloseOpenModal}>
                <View style={{ width: "100%", height: 500 }}>
                    <Image source={{ uri: post.images[0] }} style={{ width: '100%', height: '100%', resizeMode: "contain" }} />
                </View>
            </Modal>
        </Card>
    )
