...
export function BtnFavoriteJob({ id, bg = false }) {
  const auth = getAuth();
  ...
  const addFavorite = async () => {
    try {
      ...
      await setDoc(doc(db, "favoritesJobs", idFavorite), data);
      setIsFavorite(true);
      onReload();
    } catch (error) {
      console.log(error);
    ...
    try {
      ...
      if (response) {
        forEach(response, async (item) => {
          await deleteDoc(doc(db, "favoritesJobs", item.id));
        });
      }
  ...
  return (
    <View style={bg ? styles.bgWhite : styles.content}>
