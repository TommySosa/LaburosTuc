...
export function UserLoggedScreen({ idUser }) {
    ...
    const selectedComponent = (key) => {
        if (key === "cv") {
            setRenderComponent(<ChangeCvForm onClose={onCloseOpenModal} onReload={onReload} />)
        }
        if (key === "description") {
            setRenderComponent(<ChangeDescriptionForm onClose={onCloseOpenModal} onReload={onReload} />)
        }

        if (key === "education") {
            setRenderComponent(<ChangeEducationForm onClose={onCloseOpenModal} onReload={onReload} />)
        }

        if (key === "avalaibility") {
            setRenderComponent(<ChangeAvalaibilityForm onClose={onCloseOpenModal} onReload={onReload} />)
        }

        if (key === "interest") {
            setRenderComponent(<ChangeInterestsForm onClose={onCloseOpenModal} onReload={onReload} />)
        }

        if (key === "phone") {
            setRenderComponent(<ChangePhoneForm onClose={onCloseOpenModal} onReload={onReload} />)
        }
    ...
    return (
        <ScrollView>
            ...
            <PdfRead isVisible={isModalVisible} onClose={() => setModalVisible(false)} cv={userInfo.cv} fileName={"tomassosa"} />
            ...
            <View style={styles.content}>
                ...
                />
                {
                    idUser === currentUser.uid && <TouchableOpacity style={styles.editButton} onPress={() => selectedComponent("cv")}>
                        <View style={styles.circle}>
                            <Icon type='material-community' name='pencil' color='#fff' size={20} />
                        </View>
                    </TouchableOpacity>
                }
            ...
            {map(menuOptions, (menu, index) => (
                ...
                >
                    ...
                    <ListItem.Content>
                        ...
                        {
                            textLoading ? <Skeleton height={20} skeletonStyle={{ backgroundColor: "#ccc" }} /> : <Text>
                                {menu.text ? menu.text : currentUser.uid == userInfo.idUser ? <Text style={styles.inputMessage}>Aún no terminaste de configurar tu perfil!</Text> : <Text style={styles.inputMessage}>No hay nada para ver aquí</Text>}
                            </Text>
                        }
                    </ListItem.Content>
                    {
                        idUser === currentUser.uid && <Icon type={menu.iconType} name={"square-edit-outline"} color={menu.iconColorRight} onPress={menu.onPress} />
                    }
                </ListItem>
            ))}
